...
def process_jsonl(jsonl_path, output_dir):
    ...
    try:
        ...
        for i, story in enumerate(tqdm(stories, desc="Saving stories")):
            with open(os.path.join(outputdir, f"story{i+1:04d}.txt"), 'w', encoding='utf-8') as f:
                f.write(story)
...
def analyze_stories(processed_dir):
    ...
    story_files = [f for f in os.listdir(processeddir) 
                  if f.startswith('story') and f.endswith('.txt')]
    ...
    for source in sources:
        ...
        if source.get('direct_text'):
            ...
            for i, story in enumerate(tqdm(stories)):
                with open(os.path.join(processeddir, f"story{i+1:04d}.txt"), 'w') as f:
                    f.write(story)
            ...
            processed = False
            for root, , files in os.walk(extractdir):
                for file in files:
                    ...
                    if file.endswith('.jsonl'):
                        ...
                        for i, story in enumerate(stories):
                            with open(os.path.join(processeddir, f"story{i+1:04d}.txt"), 'w') as f_out:
                                f_out.write(story)
                        processed = True
